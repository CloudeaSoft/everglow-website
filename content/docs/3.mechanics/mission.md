...
任务系统通过 `PacketResolver` 统一管理网络数据包, 分为两层:

- **Send (原版 TML)**:  
  仅在当前世界内广播, 不跨世界.

- **Route (跨世界)**:  
  结合 TML 与 SubworldLibrary, 在主世界服务器、主世界客户端、子世界服务器、子世界客户端之间进行跨世界传输.  
  Route 是任务系统跨世界同步的基础.

#### 路由目标 (RouteDestination)

在 Route 层, 任务系统使用两种路由目标:

- `MainServer` (上行)
  - 所有非主世界服务器 (子服务器、主客户端、子客户端) 均使用此路由.
  - 子世界服务器只负责识别并直接转发, 不执行任务逻辑.
  - 所有包最终到达主世界服务器, 由其验证并生效.
...
1. **全任务同步**: 含所有任务的信息. 由 `ModSystem.NetSend()` 和 `ModSystem.NetReceive` 触发. 与 Terraria 本体相同, 在特定时间点全量同步世界的信息.
   - 主世界服务器使用 `AllDownstream` 广播完整任务数据.

2. **单任务同步**: 含单个任务的信息. 一般由 **数据上传** / 状态改变触发, 对所有客户端进行一次单任务同步.
   - 主世界服务器使用 `AllDownstream` 同步单个任务的最新状态.

3. **数据上传**: 含单个玩家的任务进度贡献量, 比如行走的距离, 消耗的物品等. 在客户端与子服务器上进行, 通过任务目标的 `OnMPSync` 方法, 发送数据到主服务器 (发送频率受限).
   - 客户端与子服务器均使用 `MainServer` 路由上行上传, 由主世界服务器验证后生效.

#### 发送主体

1. **主世界服务器** (广播至所有客户端和所有子世界及其客户端):  
   主世界触发世界同步时 (全任务同步), 任务生命周期改变 (单任务同步), 差量数据上传后 (单任务同步).
   - 主世界服务器是唯一可使用 `AllDownstream` 的节点.

2. **子世界服务器** (直接发送至主世界):  
   上传子世界本地收集的玩家进度 (客户端收集内容除外).
   - 始终使用 `MainServer` 路由, 仅转发, 不执行任务逻辑.

3. **客户端** (由子世界服务器转发至主世界):  
   上传客户端本地收集的玩家进度、请求重试失败任务、请求发放奖励.
   - 始终使用 `MainServer` 路由; 若处于子世界, 则由子世界服务器转发.
